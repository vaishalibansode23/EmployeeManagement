...
exports.default = validateIndividual;
const primitiveTypes = new Map([[String, 'string'], [Number, 'number'], [Boolean, 'boolean'], [Symbol, 'symbol'], [Function, 'function']]);

function getPrintableType(type) {
  let prefix = '';
  let postfix = '';
  ...
  if (typeof type === 'object') {
    return prefix + JSON.stringify(type, (key, value) => typeof value === 'object' ? value : getPrintableType(value)) + postfix;
  }

  return prefix + type.name + postfix;
}

function validateIndividual(givenType, value, index, types, values, isObjectProperty) {
  let type = givenType;
  ...
  if (type.type === 'var-args') {
    ...
    try {
      values.slice(index).forEach((item, itemIndex) => validateIndividual(type.of, item, itemIndex + index, types, values, isObjectProperty));
    } catch (error) {
  ...
  if (type.type === 'instance') {
    if (!(value instanceof type.of)) {
      throw new Error(` to be an instance of ${getPrintableType(type)}. Got: ${value ? value.name || value.constructor.name : typeof value}`);
    }
